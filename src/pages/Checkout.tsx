...
const CheckoutForm = ({ clientSecret }: { clientSecret: string }) => {
  ...
  const formattedTotal = (calculatedTotal / 100).toFixed(2);
...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    setStripePromise(loadStripe('pk_test_51O...')); // Replace with your test public key
              ...
              {items.map((item, index) => (
                <div key={`${item.product.id}-${index}`} className="order-item">
                  <div>
                    ...
                    {item.product.size && (
                      <span className="text-sm text-gray-400 block">Size: {item.product.size}</span>
                    )}
